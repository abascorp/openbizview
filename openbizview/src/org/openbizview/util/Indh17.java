...
@ManagedBean
@ViewScoped
public class Indh17 extends Bd implements Serializable {
...
/**
	...
	private LazyDataModel<Indh17> lazyModel;  
	
	
	/**
	 * @return the lazyModel
	 */
	public LazyDataModel<Indh17> getLazyModel() {
		return lazyModel;
	}	
...
public void init() {
	...
	lazyModel  = new LazyDataModel<Indh17>(){
		/**
		 * 
		 */
        ...
        @Override
		public List<Indh17> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) { 
        	//Filtro
        	if (filters != null) {
	...
	public String gettotrat() {
		return totrat;
	}

	public void settotrat(String totrat) {
		this.totrat = totrat;
	}
	...
	public String getZtotrat() {
		return ztotrat;
	}

	public void setZtotrat(String ztotrat) {
		this.ztotrat = ztotrat;
	}
...
public void insert() throws  NamingException {   	
    ...
    try {
        ...
        String query = "INSERT INTO Indh17 VALUES (?,?,trim(?),ROUND(?,2),ROUND(?,2),?,'" + getFecha() + "',?,'" + getFecha() + "',?)";
        ...
        pstmt.setFloat(4, Float.parseFloat(totusd));
        pstmt.setFloat(5, Float.parseFloat(totrat));
...
public void delete() throws NamingException  {  
	...
	if (chkbox==null){
        ...
        try {
        	...
        	String query = "DELETE from Indh17 WHERE CODCIA || ANOCAL || SEMCAL in (" + param + ") and INSTANCIA = " + instancia + "";
	...
	//System.out.println("entre al metodo UPDATE");
     try { 	 
	   ...
	   String[] vecsem = semcal.split("\\ - ", -1);
        ...
        String query = "UPDATE Indh17";
         query += " SET totusd = ROUND(?,2), ";
         query += " totrat = ROUND(?,2), ";
         query += " USRACT = ?,";
         query += " FECACT = '" + getFecha() + "'";
         query += " WHERE CODCIA = ? AND ANOCAL = ? AND SEMCAL = trim(?) AND INSTANCIA = " + instancia + "";

        pstmt = con.prepareStatement(query);
        pstmt.setFloat(1, Float.parseFloat(totusd));
        pstmt.setFloat(2, Float.parseFloat(totrat));
    ...
    public void counter(Object filterValue) throws SQLException, NamingException {
        ...
        try {	
	        ...
	        if(semcal==null){
     		...
     		//Consulta paginada
     		String query = "SELECT COUNT_INDH17('" + ((String) filterValue).toUpperCase() + "','" + veccia[0] + "','" + anocal + "','" + vecsem[0] + "','" + instancia + "') FROM DUAL";
