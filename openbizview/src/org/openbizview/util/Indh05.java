...
@ManagedBean
@ViewScoped
public class Indh05 extends Bd implements Serializable {
...
/**
	...
	private LazyDataModel<Indh05> lazyModel;  
	
	
	/**
	 * @return the lazyModel
	 */
	public LazyDataModel<Indh05> getLazyModel() {
		return lazyModel;
	}	
...
public void init() {
	...
	lazyModel  = new LazyDataModel<Indh05>(){
		/**
		 * 
		 */
        ...
        @Override
		public List<Indh05> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) { 
        	//Filtro
        	if (filters != null) {
	...
	private String usdkgr = "";
	private Object filterValue = "";
	private List<Indh05> list = new ArrayList<Indh05>();
	...
	public String getZsecdesc() {
		return zsecdesc;
	}

	public void setZsecdesc(String zsecdesc) {
		this.zsecdesc = zsecdesc;
	}
	...
	public String getUsdkgr() {
		return usdkgr;
	}

	public void setUsdkgr(String usdkgr) {
		this.usdkgr = usdkgr;
	}
	...
	public String getZusdkgr() {
		return zusdkgr;
	}

	public void setZusdkgr(String zusdkgr) {
		this.zusdkgr = zusdkgr;
	}
...
public void insert() throws  NamingException {   	
    ...
    try {
        ...
        String query = "INSERT INTO Indh05 VALUES (?,?,?,?,trim(?),ROUND(?,2),ROUND(?,2),?,'" + getFecha() + "',?,'" + getFecha() + "',?)";
        ...
        pstmt.setFloat(7, Float.parseFloat(usdkgr));
...
public void delete() throws NamingException  {  
	...
	if (chkbox==null){
        ...
        try {
        	...
        	String query = "DELETE from Indh05 WHERE CODCIA || CODUBI || CODSEC || ANOCAL || SEMCAL in (" + param + ") and INSTANCIA = " + instancia + "";
	...
	//System.out.println("entre al metodo UPDATE");
     try { 	 
	   ...
	   String[] vecsem = semcal.split("\\ - ", -1);
        ...
        String query = "UPDATE Indh05";
         query += " SET totpes = ROUND(?,2), ";
         query += " USDKGR = ROUND(?,2), ";
         query += " USRACT = ?,";
         query += " FECACT = '" + getFecha() + "'";
         query += " WHERE CODCIA = ? AND CODUBI = ? AND CODSEC = ? AND ANOCAL = ? AND SEMCAL = trim(?) AND INSTANCIA = " + instancia + "";

        pstmt = con.prepareStatement(query);
        pstmt.setFloat(1, Float.parseFloat(totpes));
        pstmt.setFloat(2, Float.parseFloat(usdkgr));
        pstmt.setString(3, login);
        pstmt.setString(4, veccia[0].toUpperCase());
        pstmt.setString(5, vecubi[0].toUpperCase());
        pstmt.setString(6, vecsec[0].toUpperCase());
    ...
    public void counter(Object filterValue) throws SQLException, NamingException {
        ...
        try {	
	        ...
	        if(codsec==null){
     		...
     		//Consulta paginada
     		String query = "SELECT COUNT_INDH05('" + ((String) filterValue).toUpperCase() + "','" + veccia[0] + "','" + vecubi[0] + "','" + vecsec[0] + "','" + anocal + "','" + vecsem[0] + "','" + instancia + "') FROM DUAL";
